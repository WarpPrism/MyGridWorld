...
public class Jigsaw {
    ...
    public boolean BFSearch() throws IOException {
        ...
        isCompleted = false;
        currentJNode = getBeginJNode();
        openList.add(currentJNode);

        while (!openList.isEmpty()) {
            JigsawNode v = openList.get(0);
            currentJNode = v;
            if (v.equals(getEndJNode())) {
                isCompleted = true;
                calSolutionPath();
                break;
            } else {
                openList.remove(v);
                closeList.add(v);
                Vector<JigsawNode> adjacent = new Vector<>();
                adjacent = findFollowJNodes(v);
                openList.addAll(adjacent);
            }
            searchedNodesNum++;
        }
     ...
     */
    /*估价函数f(n)用来估计节点n的重要性，表示为：从起始节点，经过节点n，到达目标节点的代价。f(n)越小，表示节点n越优良，
    应该优先访问它的邻接节点。可参考的估价方法有：
    1） 所有 放错位的数码 个数
    ...
    private void estimateValue(JigsawNode jNode) {
        ...
        if (dimension == 5) {
            if (jNode.getNodesState()[25] != 24 && jNode.getNodesState()[25] != 20) {
                f2 += 10;
                f1 += 10;
            }
        }
        // F1 and F2 make sure that the node is the end state.
        for (int index = 1; index < dimension * dimension; index++) {
            if (jNode.getNodesState()[index] + 1 != jNode.getNodesState()[index + 1] && index != 24) {
                f1++;
            }
            ...
            // Calculate distance.
            int x1, y1, x2, y2;
            int goal = jNode.getNodesState()[index];
            if (goal % dimension == 0) {
                x1 = 5;
                y1 = goal / 5;
            } else {
            ...
            manhattan = manhattan + deltaX + deltaY;
            euclidean = (int) (euclidean + Math.sqrt(deltaX ^ 2 + deltaY ^ 2));
        }
        f3 = (int) (manhattan);
        fn = (int) (0.2 * f1 + 0.2 * f2 + 0.6 * f3);
